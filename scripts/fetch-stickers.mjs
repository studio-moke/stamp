...
function toAbsoluteUrl(href) {
  try { return new URL(href, AUTHOR_URL).toString(); } catch { return ""; }
}
  ...
  try {
    ...
    for (const pattern of titlePatterns) {
      ...
      if (match) {
        ...
        if (title) { product.title = title; break; }
      }
    }
    ...
    for (const pattern of descriptionPatterns) {
      ...
      if (match) {
        ...
        if (description) { product.description = description; break; }
      }
    }
    ...
    for (const pattern of imagePatterns) {
      const match = html.match(pattern);
      if (match) { imageUrl = decodeHtml(match[1]); break; }
    }
    if (!imageUrl) {
...
function loadExistingProducts() {
  ...
  try {
    ...
    console.log(`既存データの読み込みに失敗しました: ${error.message}`);
    return [];
  }
}
...
async function main() {
  ...
  // LINE STORE側の一覧が一時的に少なくても、既存カタログを絶対に捨てません。
  // 新しく取得できた商品だけ既存データへ追加・更新します。
  const mergedProducts = mergeProducts(existingProducts, fetchedProducts);

  console.log(`統合後: ${mergedProducts.length}作品`);
  console.log("");

  if (mergedProducts.length < MINIMUM_SAFE_COUNT) {
    throw new Error(
      `安全確認に失敗しました: 統合後 ${mergedProducts.length}件。${MINIMUM_SAFE_COUNT}件未満なので既存データを上書きしません。`
    );
  }
  ...
  for (let i = 0; i < fetchedProducts.length; i++) {
    const product = fetchedProducts[i];
    if (!product.imageUrl) {
      console.log(`[${i + 1}/${fetchedProducts.length}] 画像URLなし`);
      continue;
    }
    console.log(`[${i + 1}/${fetchedProducts.length}] ${product.title}`);
