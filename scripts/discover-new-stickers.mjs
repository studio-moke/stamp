...
function extractProductLinks(html) {
  const found = new Map();
  const pattern = /(?:https?:\/\/store\.line\.me)?\/stickershop\/product\/(\d+)(?:\/ja)?/gi;
...
function belongsToAuthor(html) {
  return new RegExp(`/stickershop/author/${AUTHOR_ID}(?:/ja)?`, "i").test(html)
    || new RegExp(`line\\.me/R/shop/sticker/author/${AUTHOR_ID}`, "i").test(html);
}
...
async function main() {
  ...
  for (const [id, url] of candidates) {
    ...
    try {
      ...
      if (!belongsToAuthor(html)) continue;
