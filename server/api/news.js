...
async function aiCopy(c, initial = false) {
  ...
  try {
    ...
    return { title:clean(parsed.title).slice(0,120), lead:clean(parsed.lead).slice(0,180), body:clean(parsed.body).slice(0,700) };
...
async function syncNews(existing) {
  ...
  const selected = missing.slice(0, initial ? 8 : 4);
