...
import { getRuntimeDigitalProduct, getRuntimeDigitalProducts, stateKey } from "./_store-products.js";
...
export default async function handler(req,res){
 try{
  ...
  if(req.method==="GET"&&action==="status"){
   const products=await getRuntimeDigitalProducts();return json(res,200,{ok:true,paymentProvider:"stripe",paymentConfigured:stripeConfigured(),priceYen:500,productCount:products.length,publishedCount:products.filter(p=>p?.published).length});
  }
  if(req.method==="GET"&&action==="catalog"){
   const products=await getRuntimeDigitalProducts();return json(res,200,{ok:true,products:products.filter(Boolean).map(p=>({id:p.id,published:p.published,assetCount:p.assetCount||0,preparedAt:p.preparedAt||""}))});
  }
  if(req.method==="POST"&&action==="admin-health"){
   if(!isAdmin(req))return json(res,401,{ok:false,error:"管理トークンが一致しません。"});return json(res,200,{ok:true,tokenSource:process.env.STORE_ADMIN_TOKEN?"STORE_ADMIN_TOKEN":"FREE_ADMIN_TOKEN",stripeConfigured:stripeConfigured()});
  }
  ...
  if(req.method==="POST"&&action==="admin-publish"){
   if(!isAdmin(req))return json(res,401,{ok:false,error:"管理トークンが一致しません。"});const b=await readBody(req);const id=cleanId(b.productId),count=Number(b.assetCount||0);const product=id?await getRuntimeDigitalProduct(id):null;if(!product)return json(res,404,{ok:false,error:"商品が見つかりません。"});if(!Number.isInteger(count)||count<1||count>80)return json(res,400,{ok:false,error:"画像点数が不正です。"});const key=zipKey(id);if(!(await r2Head(key)))return json(res,409,{ok:false,error:"ZIPのアップロードを確認できません。"});const record={productId:id,zipKey:key,assetCount:count,published:b.published!==false,preparedAt:new Date().toISOString(),licenseVersion:"2026-09-07"};await r2PutJson(stateKey(id),record);return json(res,200,{ok:true,product:{id,published:record.published,assetCount:count,preparedAt:record.preparedAt}});
  }
  if(req.method==="POST"&&action==="admin-unpublish"){
   if(!isAdmin(req))return json(res,401,{ok:false,error:"管理トークンが一致しません。"});const b=await readBody(req);const id=cleanId(b.productId),product=id?await getRuntimeDigitalProduct(id):null;if(!product)return json(res,404,{ok:false,error:"商品が見つかりません。"});const current=await r2GetJson(stateKey(id),{});await r2PutJson(stateKey(id),{...current,productId:id,published:false,updatedAt:new Date().toISOString()});return json(res,200,{ok:true});
  }
