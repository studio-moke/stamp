...
export default async function handler(req,res){
 try{
  ...
  if(req.method==="POST"&&action==="checkout"){
   const b=await readBody(req),product=await getRuntimeDigitalProduct(b.productId);if(!product)return json(res,404,{ok:false,error:"商品が見つかりません。"});if(!product.published||!product.zipKey)return json(res,409,{ok:false,error:"この商品はまだ販売準備中です。"});if(Number(product.priceYen)!==STORE_PRICE_YEN)return json(res,409,{ok:false,error:"商品価格を確認できません。"});if(!stripeConfigured())return json(res,503,{ok:false,error:"決済はまだ有効化されていません。"});const origin=String(process.env.STORE_ORIGIN||"").replace(/\/$/,"");if(!/^https:\/\/[^/]+$/i.test(origin))return json(res,503,{ok:false,error:"決済の戻り先が設定されていません。"});try{const session=await createStripeCheckoutSession({product,origin});return json(res,200,{ok:true,checkoutUrl:session.url});}catch(error){console.error("stripe checkout setup error",error);const detail=String(error?.message||"unknown").replace(/sk_(?:test|live)_[A-Za-z0-9_]+/g,"[redacted]");return json(res,502,{ok:false,error:process.env.VERCEL_ENV==="preview"?`Preview決済設定エラー: ${detail}`:"決済の準備に失敗しました。"});}
  }
