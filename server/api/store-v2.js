...
export default async function handler(req,res){
 try{
  ...
  if(req.method==="GET"&&action==="download"){
   const sessionId=String(req.query?.session_id||"");if(!sessionId.startsWith("cs_"))return json(res,400,{ok:false,error:"購入情報が確認できません。"});let order=await r2GetJson(orderKey(sessionId),null);if(!order&&stripeConfigured()){const session=await retrieveStripeCheckoutSession(sessionId);order=await persistPaidOrder(session);}if(!order)return json(res,409,{ok:false,pending:true,error:"決済確認中です。少ししてから再度お試しください。"});const key=safeZipKey(order.productId,order.zipKey);if(!key)return json(res,410,{ok:false,error:"ダウンロード商品を確認できません。"});return json(res,200,{ok:true,productId:order.productId,title:order.title||"stamp moke 商用素材",downloadUrl:presignStoreDownload(key,600),expiresIn:600});
  }
