...
export default async function handler(req,res){
 ...
 try{
  const rawBody=await readRawBody(req);const signature=req.headers["stripe-signature"];
  if(!verifyStripeWebhookSignature(rawBody,signature)){res.statusCode=400;return res.end("Invalid signature");}
 ...
 }catch(error){console.error("store webhook v2 error",error);res.statusCode=400;return res.end("Webhook error");}
}
