...
import page, { normalizeFreeDetailHtml } from "../server/api/free-page-seo.js";
...
export default async function handler(req,res){installInternalUrlNormalization(res);const route=String(req.query?.route||"assets");if(route==="page"&&["zh-cn","ko"].includes(String(req.query?.locale||""))){const originalSend=res.send.bind(res);res.send=(body)=>typeof body==="string"?originalSend(normalizeFreeDetailHtml(body,String(req.query?.locale||"ja"))):originalSend(body);return pageZhKo(req,res)}const target=routes[route];if(!target)return res.status(404).json({error:"Unknown free API route"});return target(req,res)}
